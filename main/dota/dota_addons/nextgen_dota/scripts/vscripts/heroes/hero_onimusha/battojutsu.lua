...
function BattojutsuQ( keys )
    ...
    local length = (casterLoc - targetLoc):Length2D()
    ...
    if length < range and target:IsAlive() then
        ...
        caster:SetForwardVector((caster:GetOrigin() - target:GetOrigin()):Normalized())
...
function BattojutsuR( keys )
    ...
    if slashOrder == "QWE" then
         ...
         if length < range and target:IsAlive() then
            ...
            local casterAngles = caster:GetAngles()
            caster:SetAngles(0, casterAngles.y, casterAngles.z)
            ability:ApplyDataDrivenModifier(caster, target, "modifier_hirameki", {Duration = 0.1})
            ApplyDamage(DamageTable)
            if target:GetHealthPercent() < 7.5 and target:IsAlive() then
                caster:PerformAttack(target, true, true, true, false, false)
    ...
    elseif slashOrder == "QEW" then
        battoAbility:ApplyDataDrivenModifier(caster, caster, "modifier_battojutsu_final_strike_b", {Duration = 0.1})
         if length < range  and target:IsAlive()then
            local targets = FindUnitsInRadius(caster:GetTeam(), target:GetAbsOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, 
            ...
            caster:SetAngles(0, casterAngles.y, casterAngles.z)
            caster:PerformAttack(target, true, true, true, false, false)
            ability:ApplyDataDrivenModifier(caster, target, "modifier_ryukansen", {Duration = maimDuration})
            for i = 1, #targets do
    ...
    elseif slashOrder == "WQE" then
        ...
        if length < range and target:IsAlive() then
            ...
            caster:SetAngles(0, casterAngles.y, casterAngles.z)
            caster:PerformAttack(target, true, true, true, false, false)
            ability:ApplyDataDrivenModifier(caster, target, "modifier_ryumeisen", {Duration = stunDuration})
        else
            local units = FindUnitsInRadius(caster:GetTeam(), casterLoc, nil, range, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false)
    ...
    elseif slashOrder == "WEQ" then
        battoAbility:ApplyDataDrivenModifier(caster, caster, "modifier_battojutsu_final_strike_d", {Duration = 0.1})
         if length < range and target:IsAlive() then
            ...
            caster:SetAngles(0, casterAngles.y, casterAngles.z)
            caster:PerformAttack(target, true, true, true, false, false)
    		ability:ApplyDataDrivenModifier(caster, target, "modifier_rairyusen", {Duration = blindDuration})
            local targets = FindUnitsInRadius(caster:GetTeam(), target:GetAbsOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, 
                DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false)
    ...
    elseif slashOrder == "EQW" then
        ...
        if length < highRange and target:IsAlive() then
    		FindClearSpaceForUnit( caster, target:GetAbsOrigin(), false )
            caster:SetForwardVector((target:GetOrigin() - caster:GetOrigin()):Normalized())
            local casterAngles = caster:GetAngles()
            caster:SetAngles(0, casterAngles.y, casterAngles.z)
    ...
    caster:RemoveModifierByName("modifier_battojutsu")
	target:RemoveModifierByName("modifier_battojutsu_enemy")
    slashOrder = ""
end
function CreateIllusions( keys )
	...
	local vec1 = origin + sideVec * 256
	local vec2 = origin + sideVec * 128
	local vec3 = origin
	local vec4 = origin + sideVec * -128
	local vec5 = origin + sideVec * -256

	caster:Stop()
	ProjectileManager:ProjectileDodge(caster)
	caster:AddNoDraw()
	caster:AddNewModifier(caster, ability, "modifier_disabled_invulnerable", {Duration = 0.3})

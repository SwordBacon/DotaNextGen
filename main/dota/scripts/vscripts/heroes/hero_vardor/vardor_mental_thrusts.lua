...
function MentalThrustsHit(keys)
	...
	if yari_stack > 0 or caster:HasModifier("modifier_celestial") then
	...
	if target:HasModifier( modifierName2 ) then
		...
		target:SetModifierStackCount( modifierName2, ability, current_stack + 1)
		target:SetModifierStackCount( modifierName1, ability, current_stack + 1)
	else
		ability:ApplyDataDrivenModifier( caster, target, modifierName2, {} )
		ability:ApplyDataDrivenModifier( caster, target, modifierName1, {} )
